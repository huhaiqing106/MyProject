...
// 类型判断函数
export const getType = v => Object.prototype.toString.call(v).replace(/\[object\s(\w+)\]/, '$1').toLowerCase();
...
export const dateStrToMoment = (time, formatStr = 'YYYY-MM-DD hh:mm:ss') => {
  if (Array.isArray(time)) {
    return time.map(item => (
      moment(new Date(item), formatStr)
    ))
  } else {
...
//把小数前的数分成4位处理
export const cutNum = nums => { //判断Number对象是否有cutNum方法
...
export const filterTreeData = function (rows, config) {
  ...
  let filterKeys = filterValue ? flattenTreeData(rows, config).filter(item => {
    // 自定义节点配置
    const treeNodeConfig = treeNodeRender(item);
