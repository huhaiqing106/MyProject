...
export const withRoleBotton = (ButtonType, type = "ButtonGroup") => {
  let renderComponent = (type, children) => {
    return type == "ButtonGroup" ? <ButtonGroup style={{ marginBottom: '6px' }}>{children}</ButtonGroup > : <div style={{ marginBottom: '6px' }}>{children}</div>;
  }
  //按钮样式
  const style = { cursor: 'pointer', margin: type == "ButtonGroup" ? "0 0" : "0 6px 0 0" }
  return (
    renderComponent(type, ButtonType.map((item, index) => {
      return (
        <Button
...
export class WithTags extends Component {
  ...
  render () {
    return (
      <span onClick={(e) => { this.searchQuery(e, this.props?.func) }} >
        {
          (this.props.TagType.map(item => {
            return (
              <Tag closable={item.closable}
                color={item.color}
                className={this.state.active == item.name ? "active" : ""}
                onClose={(e) => { item.closeFun && item.closeFun(e, item) }}
