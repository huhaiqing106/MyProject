...
import { isObject, findFaceLevelNode, hasClass, setStyle,throttle,domTreeHasClass } from 'yss-biz'
...
export default props => {
    ...
    const getDomTreeHasClass = (dom, classNames) => {
        ...
        for (let i in classNames) {
            if(domTreeHasClass(dom,classNames[i]))
            return true
        }
        return false
    }
    ...
    const getDragDom = e => {
        ...
        setStyle(_DragDmo, { position: 'fixed', left: x + 'px', top: y + 'px', margin: "0" })
        startPosition = { x, y }
        return _DragDmo
    }
