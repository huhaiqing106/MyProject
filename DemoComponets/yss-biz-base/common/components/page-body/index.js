  ...
  <div
    className={
      !!props.className
        ...
        : props.haveBgColor
        ? 'page-body-have-bg-color layoutColumns'
        : 'layoutColumns'
    }
  >
    {props.children}
    ...
    return (
      <Fragment>
        <div className={this.props.className + ' side'} style={style()}>
          ...
          <span
            className={this.props.type == 'width' ? 'iconRight' : 'iconBottom'}
            onClick={() => {
              this.setState(
                () => {
                  return {
  ...
  <div className="plate-box" style={{ height: `calc(${props.height}` }}>
    ...
    <div className={!props.title ? 'plate-body not-title' : 'plate-body'}>
      <div className={!!props.footer ? 'have-footer' : 'not-footer'}>
        <div className="plate-body-inner">{props.children}</div>
      </div>
      {!!props.footer && <div className="plate-footer">{cloneElement(props.footer, { mainProps: (props.children || {}).props })}</div>}
    </div>
  </div>
);
